...
func TestQueries(t *testing.T) {
	...
	testQueries(t, env, queryTestOptions{baseCount: 1000}, []queryTestCase{
		...
		{2, s{`Int == 47`}, box.Query(E.Int.Equals(e.Int)), nil},
		{998, s{`Int != 47`}, box.Query(E.Int.NotEquals(e.Int)), nil},
		{498, s{`Int > 47`}, box.Query(E.Int.GreaterThan(e.Int)), nil},
		{500, s{`Int < 47`}, box.Query(E.Int.LessThan(e.Int)), nil},
		{1, s{`Int between -1 and 1`}, box.Query(E.Int.Between(-1, 1)), nil},
		{2, s{`Int between 47 and 94`}, box.Query(E.Int.Between(e.Int, e.Int*2)), nil},
		{2, s{`Int in [94|47]`, `Int in [47|94]`}, box.Query(E.Int.In(e.Int, e.Int*2)), nil},
		{998, s{`Int not in [94|47]`, `Int not in [47|94]`}, box.Query(E.Int.NotIn(e.Int, e.Int*2)), nil},

		{1, s{`Uint == 0`}, box.Query(E.Uint.Equals(0)), nil},
		{2, s{`Uint == 47`}, box.Query(E.Uint.Equals(e.Uint)), nil},
		{998, s{`Uint != 47`}, box.Query(E.Uint.NotEquals(e.Uint)), nil},
		//{498,`Uint > 47`}, box.Query(E.Uint.GreaterThan(e.Uint)), nil},
		//{500,`Uint < 47`}, box.Query(E.Uint.LessThan(e.Uint)), nil},
		//{1,`Uint between 0 and 1`}, box.Query(E.Uint.Between(0, 1)), nil},
		//{2,`Uint between 47 and 94`}, box.Query(E.Uint.Between(e.Uint, e.Uint*2)), nil},
		{2, s{`Uint in [94|47]`, `Uint in [47|94]`}, box.Query(E.Uint.In(e.Uint, e.Uint*2)), nil},
		{998, s{`Uint not in [94|47]`, `Uint not in [47|94]`}, box.Query(E.Uint.NotIn(e.Uint, e.Uint*2)), nil},
