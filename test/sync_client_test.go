...
func TestSyncAuth(t *testing.T) {
	...
	assert.NoErr(t, client.SetCredentials(objectbox.SyncCredentialsJwtId([]byte{4, 5, 6})))
	assert.NoErr(t, client.SetCredentials(objectbox.SyncCredentialsJwtAccess([]byte{4, 5, 6})))
	assert.NoErr(t, client.SetCredentials(objectbox.SyncCredentialsJwtRefresh([]byte{4, 5, 6})))
	assert.NoErr(t, client.SetCredentials(objectbox.SyncCredentialsJwtCustom([]byte{4, 5, 6})))
	assert.NoErr(t, client.Close())
}
...
func TestSyncJwtLoginValidToken(t *testing.T) {
	...
	assert.StringChannelMustTimeout(t, messages, env.defaultTimeout/10)
	assert.NoErr(t, env.SyncClient().SetCredentials(objectbox.SyncCredentialsJwtId([]byte(jwtToken))))
	assert.NoErr(t, env.SyncClient().Start())
	assert.StringChannelExpect(t, "success", messages, env.defaultTimeout)
	assert.StringChannelMustTimeout(t, messages, env.defaultTimeout/10)
}
...
func TestSyncJwtLoginInvalidToken(t *testing.T) {
	...
	assert.NoErr(t, env.SyncClient().SetCredentials(objectbox.SyncCredentialsJwtId([]byte(expiredJwtToken))))
	assert.NoErr(t, env.SyncClient().Start())
	assert.StringChannelExpect(t, "failure 43", messages, env.defaultTimeout)
	assert.StringChannelMustTimeout(t, messages, env.defaultTimeout/10)
}
...
func TestSyncUserPasswordLogin(t *testing.T) {
	...
	var password = ""
	assert.StringChannelMustTimeout(t, messages, env.defaultTimeout/10)
	assert.NoErr(t, env.SyncClient().SetCredentials(objectbox.SyncCredentialsUsernamePassword([]byte(user), []byte(password))))
	assert.NoErr(t, env.SyncClient().Start())
	assert.StringChannelExpect(t, "success", messages, env.defaultTimeout)
	assert.StringChannelMustTimeout(t, messages, env.defaultTimeout/10)
}
