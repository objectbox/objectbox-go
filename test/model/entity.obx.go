...
type testEntitySynced_EntityInfo struct {
	objectbox.Entity
	Uid uint64
}
...
// GeneratorVersion is called by ObjectBox to verify the compatibility of the generator used to generate this code
func (testEntitySynced_EntityInfo) GeneratorVersion() int {
	return 6
}

// AddToModel is called by ObjectBox during model build
func (testEntitySynced_EntityInfo) AddToModel(model *objectbox.Model) {
	model.Entity("TestEntitySynced", 8, 1967687883385423038)
	model.EntityFlags(2)
	model.Property("Id", 6, 1, 8356787138001727759)
	model.PropertyFlags(1)
	model.Property("Name", 9, 2, 6661805306974534817)
	model.EntityLastPropertyId(2, 6661805306974534817)
}

// GetId is called by ObjectBox during Put operations to check for existing ID on an object
func (testEntitySynced_EntityInfo) GetId(object interface{}) (uint64, error) {
	return object.(*TestEntitySynced).Id, nil
}

// SetId is called by ObjectBox during Put to update an ID on an object that has just been inserted
func (testEntitySynced_EntityInfo) SetId(object interface{}, id uint64) error {
	object.(*TestEntitySynced).Id = id
	return nil
}

// PutRelated is called by ObjectBox to put related entities before the object itself is flattened and put
func (testEntitySynced_EntityInfo) PutRelated(ob *objectbox.ObjectBox, object interface{}, id uint64) error {
	return nil
}

// Flatten is called by ObjectBox to transform an object to a FlatBuffer
func (testEntitySynced_EntityInfo) Flatten(object interface{}, fbb *flatbuffers.Builder, id uint64) error {
	obj := object.(*TestEntitySynced)
	var offsetName = fbutils.CreateStringOffset(fbb, obj.Name)
...
// Load is called by ObjectBox to load an object from a FlatBuffer
func (testEntitySynced_EntityInfo) Load(ob *objectbox.ObjectBox, bytes []byte) (interface{}, error) {
	if len(bytes) == 0 { // sanity check, should "never" happen
		return nil, errors.New("can't deserialize an object of type 'TestEntitySynced' - no data received")
	}
...
// MakeSlice is called by ObjectBox to construct a new slice to hold the read objects
func (testEntitySynced_EntityInfo) MakeSlice(capacity int) interface{} {
	return make([]*TestEntitySynced, 0, capacity)
}

// AppendToSlice is called by ObjectBox to fill the slice of the read objects
func (testEntitySynced_EntityInfo) AppendToSlice(slice interface{}, object interface{}) interface{} {
	if object == nil {
...
// Put synchronously inserts/updates a single object.
// In case the Id is not specified, it would be assigned automatically (auto-increment).
// When inserting, the TestEntitySynced.Id property on the passed object will be assigned the new ID as well.
...
//
// Returns: IDs of the put objects (in the same order).
// When inserting, the TestEntitySynced.Id property on the objects in the slice will be assigned the new IDs as well.
//
// Note: In case an error occurs during the transaction, some of the objects may already have the TestEntitySynced.Id assigned
// even though the transaction has been rolled back and the objects are not stored under those IDs.
//
// Note: The slice may be empty or even nil; in both cases, an empty IDs slice and no error is returned.
...
//
// Returns nil (and no error) in case the object with the given ID doesn't exist.
func (box *TestEntitySyncedBox) Get(id uint64) (*TestEntitySynced, error) {
	object, err := box.Box.Get(id)
	if err != nil {
		return nil, err
	} else if object == nil {
	...
	return object.(*TestEntitySynced), nil
}
...
func (box *TestEntitySyncedBox) GetMany(ids ...uint64) ([]*TestEntitySynced, error) {
	...
	return objects.([]*TestEntitySynced), nil
}
...
// Creates a query with the given conditions. Use the fields of the TestEntitySynced_ struct to create conditions.
// Keep the *TestEntitySyncedQuery if you intend to execute the query multiple times.
// Note: this function panics if you try to create illegal queries; e.g. use properties of an alien type.
// This is typically a programming error. Use QueryOrError instead if you want the explicit error check.
func (box *TestEntitySyncedBox) Query(conditions ...objectbox.Condition) *TestEntitySyncedQuery {
	return &TestEntitySyncedQuery{
		box.Box.Query(conditions...),
	}
}
...
func (box *TestEntitySyncedBox) QueryOrError(conditions ...objectbox.Condition) (*TestEntitySyncedQuery, error) {
	if query, err := box.Box.QueryOrError(conditions...); err != nil {
		...
		return &TestEntitySyncedQuery{query}, nil
	}
}
...
func (box *TestEntitySyncedBox) Async() *TestEntitySyncedAsyncBox {
	return &TestEntitySyncedAsyncBox{AsyncBox: box.Box.Async()}
}
...
// The returned struct must be freed explicitly using the Close() method.
// It's usually preferable to use TestEntitySyncedBox::Async() which takes care of resource management and doesn't require closing.
func AsyncBoxForTestEntitySynced(ob *objectbox.ObjectBox, timeoutMs uint64) *TestEntitySyncedAsyncBox {
	...
	return &TestEntitySyncedAsyncBox{AsyncBox: async}
}
